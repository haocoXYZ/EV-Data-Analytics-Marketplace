...
try
{
    using (SqlConnection conn = new SqlConnection(connectionString))
    {
        ...
        // 6. Kiểm tra DatasetRecords theo từng District
        ...
        using (SqlCommand cmd = new SqlCommand(@"
            SELECT dataset_id, dataset_name, moderation_status
            FROM Dataset
        ...
        using (SqlDataReader reader = cmd.ExecuteReader())
        {
            ...
            else
            {
                while (reader.Read())
                {
                    Console.WriteLine($"  • Record: {reader["RecordId"]}, Station: {reader["station_name"]}, " +
                                    $"Energy: {reader["energy_kwh"]} kWh, Status: {reader["moderation_status"]}");
                }
                Console.WriteLine();
            }
        }
